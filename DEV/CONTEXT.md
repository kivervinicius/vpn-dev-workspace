...
- OpenCode GUI: `./scripts/vpn-opencode web|serve|check|watch` (porta `OPENCODE_GUI_PORT`, padrão `10001`, na `VPN_PORT_RANGE`; `check`=ok|down|quota, `watch`=down-restart/quota-alerta-puro, tudo opt-in). Desktop App do host conecta em `http://127.0.0.1:10001` (login `opencode`; senha em `.secrets/opencode_gui_password`). Passo a passo: `RUNBOOKS/opencode-desktop-vpn.md`.
