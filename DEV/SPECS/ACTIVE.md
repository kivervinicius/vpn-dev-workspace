...
- GUI do OpenCode pela VPN: `scripts/vpn-opencode` mantém `web`/`serve` diretos e adiciona `supervise`; o serviço `opencode-supervisor` possui o processo OpenCode dentro do namespace da VPN, sem Docker socket. Queda simples reinicia só o app; desconexão ou provider rate limit/429 dispara rotação pelo Control Server do Gluetun, sonda o túnel e tenta outro servidor quando necessário até recuperar conectividade ou atingir cooldown/janela/máximo de rotações. Antes de derrubar o processo, o supervisor registra a sessão mais recente; após a recuperação confirma que o mesmo ID continua disponível na home persistente. O banner/erro de franquia gratuita (`FreeUsageLimitError`, `free_tier_limit`, `Free limit reached`, `subscribe to Go`) vira `free_tier_limit`; `Go limit reached`/`account_rate_limit` continua como `account_limit`. Ambos são tratados sem rotação inútil de VPN. `OPENCODE_AUTOCHECK` permanece apenas observacional.
...
- `vpn-opencode check` distingue `ok|down|quota` (`--json` com `{status,port,detail}`); `watch` é compatibilidade para iniciar o `opencode-supervisor` e seguir logs. O supervisor classifica `free_tier_limit|account_limit|rate_limit|disconnect`: `rate_limit`/desconexão rotacionam a VPN; `free_tier_limit` e `account_limit` não. Em recuperação de conectividade, o OpenCode só reinicia após `vpn-check --only tunnel` ficar saudável; o contexto da sessão capturada é verificado após o restart sem reenviar automaticamente a requisição interrompida. Com `OPENCODE_AUTOCHECK=true`, o `vpn-auto-reconnect` continua apenas observacional.
