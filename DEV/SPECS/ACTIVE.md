...
- GUI do OpenCode pela VPN: `scripts/vpn-opencode` mantém `web`/`serve` diretos e adiciona `supervise`; o serviço `opencode-supervisor` possui o processo OpenCode dentro do namespace da VPN, sem Docker socket. Queda simples reinicia só o app; desconexão ou provider rate limit/429 dispara rotação pelo Control Server do Gluetun, aguarda túnel saudável e sobe novamente o OpenCode, com cooldown/janela/máximo de rotações. O banner/erro de franquia gratuita (`FreeUsageLimitError`, `free_tier_limit`, `Free limit reached`, `subscribe to Go`) vira `free_tier_limit`; `Go limit reached`/`account_rate_limit` continua como `account_limit`. Ambos são tratados sem rotação inútil de VPN. `OPENCODE_AUTOCHECK` permanece apenas observacional.
